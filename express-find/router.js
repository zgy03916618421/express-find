...
/*router.get('/title/:name',function (req,res) {
    ...
    var searchUrl=encodeURI('https://www.amazon.cn/s/ref=nb_sb_noss?__mk_zh_CN=%E4%BA%9A%E9%A9%AC%E9%80%8A%E7%BD%91%E7%AB%99&url=search-alias%3Dstripbooks&field-keywords='+key);
    ...
    isbn=result['Amazon'].Barcode.trim();
    result['DOUBAN']=DBF(isbn);
    TCNMARC(key,result,function (rs) {
        res.json(rs);
    })
})*/
router.get('/:id',function (req,res) {
    ...
    var result={};
    result['Amazon']=detailF(body);
    var isbn=result['Amazon'].Barcode.trim();
    result['DOUBAN'] = DBF(isbn);
    CNMARC(isbn,result,function (rs) {
        res.json(rs);
    });
});
...
/*function TCNMARC(key,result,cb) {
    var url='mongodb://192.168.1.198:27017/bookshelf';
    MongeClient.connect(url,function (err,db) {
        console.log('connected sucess!!');
        TfindDoc(db,key,result,function (rs) {
            cb(rs);
        })
    })
}
function TfindDoc(db,key,result,cb) {
    var collection=db.collection('bookdetail');
    collection.find({'title':key}).toArray(function (err,docs) {
        result['CNMARC']=docs;
        cb(result);
    })
}*/
